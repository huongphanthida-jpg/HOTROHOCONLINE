...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handlePushAppDataCloud = async () => {
    ...
    if (!url || !url.startsWith('http')) {
      ...
      setCloudSyncMsg('Vui lòng nhập URL Google Apps Script Web App hợp lệ.');
      return;
    }
    try {
      await fetch(url, {
      ...
      setCloudSyncMsg('Đã phát tín hiệu đồng bộ toàn bộ dữ liệu ứng dụng tới Google Sheets thành công!');
    } catch (err: any) {
      setCloudSyncStatus('error');
      setCloudSyncMsg(`Lỗi kết nối: ${err?.message || err}`);
    }
  };
  ...
  const handlePullAppDataCloud = async () => {
    ...
    try {
      const res = await fetch(`${url}?action=getFullAppData&t=${Date.now()}`);
      if (res.ok) {
        ...
        if (json.status === 'success' && json.appData) {
          const merged: AppData = {
            ...appData,
            ...json.appData,
            subjects: json.appData.subjects || appData.subjects,
            questions: json.appData.questions || appData.questions,
            documents: json.appData.documents || appData.documents,
            games: json.appData.games || appData.games,
            onlineClasses: json.appData.onlineClasses || appData.onlineClasses,
          };
          onImportData(merged);
          setCloudSyncStatus('success');
      ...
      setCloudSyncStatus('error');
      setCloudSyncMsg('Chưa tìm thấy bản sao lưu dữ liệu trên Google Sheets hoặc link Web App chưa bật action=getFullAppData.');
    } catch (err: any) {
      setCloudSyncStatus('error');
      setCloudSyncMsg(`Không thể kết nối đến Google Apps Script: ${err?.message || err}`);
    }
  };
